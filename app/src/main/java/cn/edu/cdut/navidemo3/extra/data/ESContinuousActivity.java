 ...
 *
 *ESContinuousActivity表示持续时间超过一分钟的事件，
 * *和对应于一个连续的ESActivity记录/对象数组
 * *它们都有完全相同的标签(主要，次要和情绪)。
 * *如果活动有用户纠正主活动标签，服务器预测将被忽略。
...
public class ESContinuousActivity {
     ...
     * *如果用户对这个连续活动的主活动有更正，返回它。
     * *否则，返回您为该连续活动找到的服务器预测。
     * * @return此连续活动的最新主活动标签。
     */
...
//        }
    ...
    public double[] getLocationLatLong() {
        if (this.isEmpty()) {
            return null;
        }
        for (ESActivity minuteActivity : _minuteActivities) {
            if (minuteActivity != null) {
                //String correction = minuteActivity.get_mainActivityUserCorrection();
                double[] locationLatLong = minuteActivity.get_locationLatLong();
    ...
    public String toString() {
        return "<start-timestamp: " + getStartTimestamp() +
                ...
                ",mood: {" + getMoods() + "}"
                //+",position: {" + getLocationLatLongFromFirstInstance()+"," + "}"
                ;
    }
    // Static interface:
