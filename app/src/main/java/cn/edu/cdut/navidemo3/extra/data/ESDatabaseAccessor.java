...
import cn.edu.cdut.navidemo3.ui.gallery.GalleryFragment;
...
public class ESDatabaseAccessor {
    ...
    public static ESDatabaseAccessor getESDatabaseAccessor() {
        if (_theSingleAccessor == null) {
            if (ESApplication.getTheAppContext()!=null) {
                _theSingleAccessor = new ESDatabaseAccessor(ESApplication.getTheAppContext());
            }else {
            _theSingleAccessor = new ESDatabaseAccessor(MainActivity.getInstance());
            }
        }
    ...
    /**
     * *获取与给定时间戳相对应的ESActivity实例;
     * 如果不存在这样的记录，则为null。
     * *从DB中读取相应记录的属性，并返回一个具有这些属性的对象。
     * *
     * * @返回所需的活动，如果没有时间戳记录，则为空。
    ...
    public synchronized void setESActivityUserCorrectedValuesAndPossiblySendFeedback(ESActivity activity, ESActivity.ESLabelSource labelSource,
        ...
        setESActivityValuesAndPossiblySendFeedback(activity,labelSource,
                ...
                GalleryFragment.getCurrentPositionDoubleArray(),
                timestampOpenFeedbackForm,timestampPressSendButton,
                timestampNotification,timestampUserRespondToNotification,
                sendFeedback);
     ...
     * IFF sendFeedback: after setting the new values, this will trigger an API call to send the labels to the server.
     **更改活动实例的属性值。
     * *这些变化将反映在给定的ESActivity对象
     * *和数据库中相应的记录。
     * * IFF sendFeedback:设置新值后，这将触发API调用发送标签到服务器。
     * @param activity The activity instance to set
     * @param labelSource The label source value to assign to the activity
     * @param mainActivityServerPrediction The server prediction to assign to the activity
     * @param mainActivityUserCorrection The user correction to assign to the activity
    ...
    private synchronized void setESActivityValuesAndPossiblySendFeedback(ESActivity activity,ESActivity.ESLabelSource labelSource,
        ...
        SQLiteDatabase db = _dbHelper.getWritableDatabase();
        //locationLatLong = GalleryFragment.getCurrentPositionDoubleArray();
        ...
        //if (cursor.getString(cursor.getColumnIndexOrThrow(ESDatabaseContract.ESActivityEntry.COLUMN_NAME_LOCATION_REPRESENTATIVE_LAT_LONG_CSV)));
        Cursor cursor = db.query(ESDatabaseContract.ESActivityEntry.TABLE_NAME,projection ,selection,null,null,null,null);
        cursor.moveToFirst();
        String pre_locationLatLongCSV = cursor.getString(cursor.getColumnIndexOrThrow(ESDatabaseContract.ESActivityEntry.COLUMN_NAME_LOCATION_REPRESENTATIVE_LAT_LONG_CSV));
        cursor.close();
        double[] d = new double[] {0.0,0.0};
        if (pre_locationLatLongCSV.equals("0.0,0.0")){
            ...
            Log.i("asdYESSSS" ,"yessss " + pre_locationLatLongCSV +" " + (pre_locationLatLongCSV.equals("0.0,0.0")));
        }
     ...
     *
     * *获取给定时间范围内的所有活动，已合并为连续活动。
     * *此方法将从数据库中提取相关信息
     * *并返回一个相应对象的数组，按时间(timestamp)升序排列。
     *
     ...
     *
     * 我们是否应该添加虚拟活动来表示连续活动之间的间隔，这些活动在时间上是分开的?
        从期望的时间范围连续活动的数组，按时间升序排列
     */
     ...
     * *此方法将从数据库中提取相关信息
     * *并返回一个相应对象的数组，按时间(timestamp)升序排列。
     * * fromTimestamp指定范围内最早的时间
     * * toTimestamp在指定范围内的最新时间
     * 按时间升序排序的期望活动数组。
     ...
     * *构造一个ESActivity对象，表示数据库记录中的活动。
     * 这个方法假设游标处于活动状态并且指向一条记录。
     * * @param cursor当前指向所需记录的游标。
     * * @return当前记录指向的ESActivity对象。
     *
     */
    private synchronized ESActivity extractActivityFromCurrentRecord(Cursor cursor) {
        ...
        //Log.i("ASDF",""+moods.length +" "+ moods[0]);
        ...
        //double[] locationLatLong = GalleryFragment.getCurrentPositionDoubleArray();
        //Log.i("ASDF",""+locationLatLong[0]+","+locationLatLong[1]); //22.803296,113.952026

        //这是我加的
        //String locationLatLongCSV = ESLabelStrings.makeCSV(locationLatLong);
        //(new ContentValues()).put(ESDatabaseContract.ESActivityEntry.COLUMN_NAME_LOCATION_REPRESENTATIVE_LAT_LONG_CSV,locationLatLongCSV);
